...
- [ ] Nahradené v `public/assets/img/`, prípadne doplnené `<picture>` s PNG zálohou v `tools/gen.py`
...
|---|---|---|
| 5 | `public/robots.txt` má riadok so sitemapou a generuje sa spolu so stránkami | `tools/gen.py`, `public/robots.txt` |
| 6 | `public/sitemap.xml` vytvorená, generuje sa z jedného zoznamu | `tools/gen.py`, `public/sitemap.xml` |
| 7 | vlastná stránka 404 vo vizuáli webu, s odkazmi Domov / Cenník / Demo | `404.html` |
| 12 | `og:image` 1200 × 630, `og:url`, `og:site_name`, `twitter:card` | `tools/obrazky.py`, `public/assets/img/og-gridservis.png` |
